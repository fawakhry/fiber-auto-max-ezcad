// MATBAAGY FIBER LASER 5.6.2 FINAL FIX
...
function autoLightGray(gray,mask,w,h,face,p){
  ...
  const r=clamp(Math.round(Math.min(w,h)*(.055+s*.045)),22,150),local=localMap(gray,mask,w,h,r,false,p),stats=grayPercentiles(gray,mask,.018,.988),target=clamp(stats.mean<118?142:stats.mean>168?150:146,136,154),tmp=new Uint8Array(gray.length);tmp.fill(255);
  ...
  const st=grayPercentiles(tmp,mask,.025,.975),out=new Uint8Array(gray.length);out.fill(255);const stretch=.34+.44*s;
...
function diffusePhoto(tone,mask,w,h,density,face){
  const work=Float32Array.from(tone),out=new Uint8Array(tone.length);out.fill(255);
  const kernel=[[1,0,7],[2,0,5],[-2,1,3],[-1,1,5],[0,1,7],[1,1,5],[2,1,3],[-2,2,1],[-1,2,3],[0,2,5],[1,2,3],[2,2,1]],divisor=48;
  const threshold=112+(density-5)*3.2;
  for(let y=0;y<h;y++){
    const reverse=y%2===1,start=reverse?w-1:0,end=reverse?-1:w,step=reverse?-1:1;
    for(let x=start;x!==end;x+=step){
      const i=y*w+x;if(!mask[i])continue;
      let v=clamp(work[i],0,255);if(face&&inFace(x,y,face))v=clamp(v+6,0,255);
      const value=v<threshold?0:255;out[i]=value;
      const err=v-value;
...
function reinforcePhotoEdges(binary,tone,mask,w,h,face,detail,edgeSmooth){
  const limit=(46+edgeSmooth*4)-detail*2;
  for(let y=1;y<h-1;y++)for(let x=1;x<w-1;x++){
    const i=y*w+x;if(!mask[i])continue;
    const gx=-tone[i-w-1]-2*tone[i-1]-tone[i+w-1]+tone[i-w+1]+2*tone[i+1]+tone[i+w+1],gy=-tone[i-w-1]-2*tone[i-w]-tone[i-w+1]+tone[i+w-1]+2*tone[i+w]+tone[i+w+1],edge=(Math.abs(gx)+Math.abs(gy))/4;
    if(edge>limit&&tone[i]<226)binary[i]=0;
  }
  return binary;
}
function mergePhotoLayers(soft,hard,tone,mask,w,h,face){
  for(let i=0;i<soft.length;i++){
    if(!mask[i]){soft[i]=255;continue}
    if(hard[i]===0&&tone[i]<(face&&inFace(i%w,Math.floor(i/w),face)?104:118))soft[i]=0;
  }
  return soft;
}
function cleanPhotoDither(binary,mask,w,h,strength){
  if(strength<4)return binary;
  const copy=binary.slice(),need=strength>=8?2:1;
  for(let y=1;y<h-1;y++)for(let x=1;x<w-1;x++){
    const i=y*w+x;if(!mask[i]||copy[i]!==0)continue;
    let n=0;for(let yy=-1;yy<=1;yy++)for(let xx=-1;xx<=1;xx++)if((xx||yy)&&copy[i+yy*w+xx]===0)n++;
    if(n<need)binary[i]=255;
  }
  return binary;
}
function resizeToneBilinear(tone,mask,w,h,nw,nh){
  const out=new Float32Array(nw*nh),sx=w/nw,sy=h/nh;
  for(let y=0;y<nh;y++)for(let x=0;x<nw;x++){
    const fx=x*sx,fy=y*sy,x0=Math.floor(fx),y0=Math.floor(fy),x1=Math.min(w-1,x0+1),y1=Math.min(h-1,y0+1),tx=fx-x0,ty=fy-y0;
    const v00=mask[y0*w+x0]?tone[y0*w+x0]:255,v10=mask[y0*w+x1]?tone[y0*w+x1]:255,v01=mask[y1*w+x0]?tone[y1*w+x0]:255,v11=mask[y1*w+x1]?tone[y1*w+x1]:255;
    const top=v00+(v10-v00)*tx,bot=v01+(v11-v01)*tx;
    out[y*nw+x]=top+(bot-top)*ty;
  }
  return out;
}
function scaleFace(face,ow,oh,nw,nh){
  if(!face)return null;
  return{x:Math.round(face.x*nw/ow),y:Math.round(face.y*nh/oh),w:Math.max(1,Math.round(face.w*nw/ow)),h:Math.max(1,Math.round(face.h*nh/oh)),detected:!!face.detected};
}
...
async function processMask(mask){
  setProgress(48,"قص ذكي محكم حول الشخص بدون فريم أبيض زائد…");await tick();const crop=cropPerson(mask);setProgress(57,"تحديد الوجه وحماية الملامح…");await tick();const face=await faceRect(crop),data=crop.source.getContext("2d",{willReadFrequently:true}).getImageData(0,0,crop.w,crop.h),gray=grayscale(data,crop.mask),p=currentPreset(),q=qualitySettings();
  setProgress(66,"توزيع الإضاءة تلقائيًا ورفع التباين داخل الشخص…");await tick();const balanced=autoLightGray(gray,crop.mask,crop.w,crop.h,face,p);
  setProgress(72,"تحويل الأسود/الأبيض مع حماية ملامح الوجه…");await tick();const f=Math.max(1,Math.round(crop.subject.area/900000)),noiseBoost=1+q.cleanup*.10+(10-q.faceDetail)*.035,hard=cleanComponents(binarize(balanced,crop.mask,crop.w,crop.h,face,p),crop.mask,crop.w,crop.h,face,Math.round(p.noise*f*noiseBoost),Math.round(p.holes*f*(1+q.edgeSmooth*.04)));
  setProgress(80,"تحجيم الملف على مقاس EZCAD والـ DPI المختار…");await tick();const target=readTargetSize(crop.w,crop.h),targetTone=resizeToneBilinear(balanced,crop.mask,crop.w,crop.h,target.w,target.h),targetMask=resizeMaskNearest(crop.mask,crop.w,crop.h,target.w,target.h),targetFace=scaleFace(face,crop.w,crop.h,target.w,target.h);
  setProgress(84,"بناء التدرج الرمادي الناعم بنقاط الحفر…");await tick();const soft=diffusePhoto(targetTone,targetMask,target.w,target.h,q.blackStrength,targetFace);
  setProgress(90,"تقوية تفاصيل الوجه والشعر والحواف وتنظيف النويز…");await tick();reinforcePhotoEdges(soft,targetTone,targetMask,target.w,target.h,targetFace,q.faceDetail,q.edgeSmooth);mergePhotoLayers(soft,resizeBinaryNearest(hard,crop.w,crop.h,target.w,target.h),targetTone,targetMask,target.w,target.h,targetFace);cleanPhotoDither(soft,targetMask,target.w,target.h,q.cleanup);Object.assign(state,{binary:soft,finalMask:targetMask,width:target.w,height:target.h,face:targetFace,inverted:false,sourceProcessSize:{w:crop.w,h:crop.h},exportMeta:{...target,material:ui.materialSelect?.value||"steel_photo"}});
  renderMask();renderFinal();if(!pureBinary())throw Error("Binary validation failed");setProgress(100,"جاهز لـ EZCAD — تدرج رمادي ناعم وتفاصيل عالية");ui.resultStats.hidden=false;ui.exportBar.hidden=false;ui.invertBtn.disabled=false;ui.sizeStat.textContent=`${target.w} × ${target.h} px`;ui.faceStat.textContent=face.detected?"تم اكتشاف الوجه وحماية ملامحه":"حماية الوجه التقديرية فعّالة";updateEzcadPanel();
}
