// MATBAAGY FIBER LASER 5.8.0 — leather photo halftone engine
...
const MATERIALS = {
  leather_wallet: {
    title: "جلد محفظة — صورة منقطة فايبر 30W",
    ...
    dpi: "333",
    loop: "1",
    note: "الملف منقّط وجاهز: Fixed DPI 333، وGray/Dither/Lighten/Pixel Power OFF، وDrill Mode ON. اختبر 8% ثم 12% ثم 16% على قصاصة من نفس الجلد؛ لا تعالج الصورة مرتين داخل EZCAD."
  },
  steel_photo: {
...
const ui = Object.fromEntries(["modeHome","photoWorkspace","eyeWorkspace","fileInput","dropZone","fileMeta","photoEnhanceBtn","prepareBtn","eyeBtn","eyeFileInput","eyeDropZone","eyeDropTitle","eyeDropHint","eyeFileMeta","eyeEnhanceBtn","eyeStyleSelect","eyeDensitySelect","eyeDetailRange","eyeFadeRange","eyeNoiseRange","eyeWidthMm","eyeDpiSelect","eyeEzcadDpi","eyeSizePreview","cleanupRange","edgeSmoothRange","lightBalanceRange","faceDetailRange","blackStrengthRange","eyeEmpty","eyePreview","eyeOriginalCanvas","modelBadge","progressWrap","progressBar","progressText","manualPanel","brushSize","resetMaskBtn","applyMaskBtn","emptyState","previewGrid","originalCanvas","maskCanvas","finalCanvas","eyeCanvas","eyeCard","maskCanvasWrap","resultStats","exportBar","invertBtn","cutoutBtn","pngBtn","bmpBtn","txtBtn","svgBtn","pdfBtn","testCardBtn","eyePngBtn","eyeSvgBtn","sizeStat","faceStat","inkStat","exportPxStat","toast","materialSelect","targetWidth","targetHeight","dpiSelect","lineSpacePreview","targetValidation","lockRatio","materialName","powerVal","speedVal","frequencyVal","lineSpaceVal","dpiVal","loopVal","settingsNote","versionLabel","updateBtn","updatePanel","closeUpdateBtn","updateTitle","updateStatus","updateDownloadLink"].map(id => [id, $(id)]));
const state = { source:null, eyeSource:null, name:"portrait", eyeName:"eye", mask:null, alpha:null, cutout:null, finalMask:null, binary:null, eyeBinary:null, eyeWidth:0, eyeHeight:0, width:0, height:0, segmenter:null, faceLandmarker:null, mattingModel:null, upscaler:null, hfModule:null, hfPromise:null, mattingPromise:null, upscalePromise:null, vision:null, visionModule:null, modelPromise:null, facePromise:null, processing:false, inverted:false, manual:false, brushMode:"add", drawing:false, lastPoint:null, face:null, inkStats:null, exportMeta:null, eyeExportMeta:null, sourceProcessSize:null, toastTimer:0 };
...
function buildSettingsText(){
  ...
  return [
    ...
    `Halftone: ${meta.halftone||"Stucki"} — already processed`,
...
function prepareLeatherTone(gray,mask,w,h,face,p,q=qualitySettings()){
  const stats=grayPercentiles(gray,mask,.012,.992),radius=clamp(Math.round(Math.min(w,h)*.022),7,40),local=localMap(gray,mask,w,h,radius,false,p),out=new Uint8Array(gray.length);out.fill(255);
  const gamma=clamp(.58+q.blackStrength*.035+(p.gamma||0),.54,.98),shadowFloor=clamp(92-q.blackStrength*5+(p.shadow||0),36,104),detail=.16+q.faceDetail*.022;
  for(let y=0;y<h;y++)for(let x=0;x<w;x++){
    const i=y*w+x;if(!mask[i])continue;
    const normalized=clamp((gray[i]-stats.lo)/Math.max(20,stats.hi-stats.lo),0,1),base=255*Math.pow(normalized,gamma),localDetail=(gray[i]-local[i])*(inFace(x,y,face)?detail*1.18:detail*.82);
    let v=shadowFloor+(255-shadowFloor)*clamp((base+localDetail)/255,0,1);
...
function diffuseLeatherPhoto(tone,mask,w,h,p,q=qualitySettings()){
  const work=Float32Array.from(tone),out=new Uint8Array(tone.length);out.fill(255),kernel=[[1,0,8],[2,0,4],[-2,1,2],[-1,1,4],[0,1,8],[1,1,4],[2,1,2],[-2,2,1],[-1,2,2],[0,2,4],[1,2,2],[2,2,1]],divisor=42;
  for(let y=0;y<h;y++){
  ...
  return limitInkTiles(out,tone,mask,w,h,clamp(.45+q.blackStrength*.02+(p.ink||0),.48,.74));
}
function reinforcePhotoEdges(binary,tone,gray,mask,w,h,face,p,q=qualitySettings()){
  ...
  for(let y=1;y<h-1;y++)for(let x=1;x<w-1;x++){
    ...
    if(edge>limit*(inFace(x,y,face) ? .72 : 1)){let neighbors=0;for(let yy=-1;yy<=1;yy++)for(let xx=-1;xx<=1;xx++)if((xx||yy)&&binary[i+yy*w+xx]===0)neighbors++;if(neighbors<6)binary[i]=0}
  }
  return limitInkTiles(binary,tone,mask,w,h,clamp(.47+q.blackStrength*.02+(p.ink||0),.50,.76));
}
...
async function processMask(mask){
  ...
  setProgress(84,"تحويل درجات الجلد إلى نقط Stucki فوتوغرافية…");await tick();const tone=prepareLeatherTone(sizedGray,finalMask,target.w,target.h,targetFace,p,q);let binary=diffuseLeatherPhoto(tone,finalMask,target.w,target.h,p,q);setProgress(92,"تعزيز العين والفم والشعر ومنع كتل الفحم…");await tick();binary=reinforcePhotoEdges(binary,tone,sourceGray,finalMask,target.w,target.h,targetFace,p,q);const inkStats=photoInkStats(binary,finalMask,target.w,target.h);Object.assign(state,{binary,finalMask,width:target.w,height:target.h,face,inkStats,inverted:false,sourceProcessSize:{w:crop.w,h:crop.h},exportMeta:{...target,material:ui.materialSelect?.value||"leather_wallet",halftone:"Stucki",inkCoverage:inkStats.coverage}});
  renderMask();renderFinal();if(!pureBinary()||inkStats.maxTileCoverage>.86)throw Error("Halftone validation failed");setProgress(100,"جاهز لـ EZCAD — درجات الصورة محفوظة كنقط منفصلة");ui.resultStats.hidden=false;ui.exportBar.hidden=false;ui.invertBtn.disabled=false;ui.sizeStat.textContent=`${target.w} × ${target.h} px`;ui.faceStat.textContent=face.detected?"تم اكتشاف الوجه وحماية ملامحه":"حماية الوجه التقديرية فعّالة";if(ui.inkStat)ui.inkStat.textContent=`كثافة النقط ${(inkStats.coverage*100).toFixed(0)}% · بدون كتل سوداء`;updateEzcadPanel();
}
...
async function exportLeatherTest(){
  const dpi=getDpi(),w=mmToPx(60,dpi),h=mmToPx(18,dpi),c=makeCanvas(w,h),ctx=c.getContext("2d",{alpha:false}),im=ctx.createImageData(w,h),d=im.data,bayer=[0,48,12,60,3,51,15,63,32,16,44,28,35,19,47,31,8,56,4,52,11,59,7,55,40,24,36,20,43,27,39,23,2,50,14,62,1,49,13,61,34,18,46,30,33,17,45,29,10,58,6,54,9,57,5,53,42,26,38,22,41,25,37,21],levels=[.10,.20,.30,.40,.50,.60,.70],margin=Math.round(w*.025),top=Math.round(h*.08),labelH=Math.round(h*.28),cellW=Math.floor((w-margin*2)/levels.length),bottom=h-labelH;
  d.fill(255);for(let p=3;p<d.length;p+=4)d[p]=255;
  levels.forEach((level,n)=>{const x0=margin+n*cellW,x1=n===levels.length-1?w-margin:x0+cellW;for(let y=top;y<bottom;y++)for(let x=x0;x<x1;x++){const black=bayer[(y&7)*8+(x&7)]<Math.round(level*64),i=(y*w+x)*4,v=black?0:255;d[i]=d[i+1]=d[i+2]=v}});ctx.putImageData(im,0,0);ctx.fillStyle="#000";ctx.textAlign="center";ctx.textBaseline="middle";ctx.font=`700 ${Math.max(10,Math.round(labelH*.48))}px Arial`;levels.forEach((level,n)=>ctx.fillText(`${Math.round(level*100)}%`,margin+n*cellW+cellW/2,h-labelH/2));
  download(await canvasToPngBlobWithDpi(c,dpi),"png",`leather-density-test-${dpi}dpi`);showToast("تم تصدير اختبار كثافة الجلد. احفره على قصاصة بطاقة 8% ثم 12% ثم 16%.");
}
...
for(const el of [ui.cleanupRange,ui.edgeSmoothRange,ui.lightBalanceRange,ui.faceDetailRange,ui.blackStrengthRange])el?.addEventListener("input",()=>invalidatePhotoResult());
...
window.__fiberAutoMax={get state(){return state},selectMode,verifyPureBinary:pureBinary,presets:Object.keys(PRESETS),materials:Object.keys(MATERIALS),version:APP_VERSION,test:{mmToPx,pxToMm,validatePixelSize,resizeGrayBilinear,autoLightGray,prepareLeatherTone,diffuseLeatherPhoto,reinforcePhotoEdges,photoInkStats,encodeBMP,canvasToPngBlobWithDpi,compareVersions}};
