...
function buildTransparentCutout(mask,alpha){
  const w=state.source.width,h=state.source.height,b=bounds(mask,w,h);if(!b)return null;
  const m=Math.max(2,Math.round(Math.max(b.w,b.h)*.025)),sx=Math.max(0,b.x-m),sy=Math.max(0,b.y-m),ex=Math.min(w,b.x+b.w+m),ey=Math.min(h,b.y+b.h+m),ow=ex-sx,oh=ey-sy,c=makeCanvas(ow,oh),ctx=c.getContext("2d",{alpha:true,willReadFrequently:true});
  ctx.drawImage(state.source,sx,sy,ow,oh,0,0,ow,oh);const im=ctx.getImageData(0,0,ow,oh),d=im.data;
  for(let y=0;y<oh;y++)for(let x=0;x<ow;x++){const si=(y+sy)*w+x+sx,p=(y*ow+x)*4,a=clamp(alpha[si]||0,0,1);if(a<.98){const k=(1-a)*.55;d[p]=clamp(Math.round(d[p]+(d[p]-255)*k),0,255);d[p+1]=clamp(Math.round(d[p+1]+(d[p+1]-255)*k),0,255);d[p+2]=clamp(Math.round(d[p+2]+(d[p+2]-255)*k),0,255)}d[p+3]=Math.round(a*255)}
  ctx.clearRect(0,0,ow,oh);ctx.putImageData(im,0,0);return c;
}
...
function cropPerson(mask){
  const w=state.source.width,h=state.source.height,b=bounds(mask,w,h);if(!b)throw Error("Empty mask");const m=Math.max(4,Math.round(Math.max(b.w,b.h)*.045)),ow=b.w+2*m,oh=b.h+2*m,sx=b.x-m,sy=b.y-m,c=makeCanvas(ow,oh),ctx=c.getContext("2d",{alpha:false,willReadFrequently:true}),cm=new Uint8Array(ow*oh);
...
function toImageData(binary,w,h,invert=false,mask=null,transparent=false){const im=new ImageData(w,h),d=im.data;for(let i=0,p=0;i<binary.length;i++,p+=4){const outside=transparent&&mask&&!mask[i],v=outside?255:(invert?255-binary[i]:binary[i]);d[p]=d[p+1]=d[p+2]=v;d[p+3]=outside?0:255}return im}

function renderMask(){
  if(!state.source||!state.mask)return;const c=ui.maskCanvas;
  if(!state.manual&&state.cutout){const s=fit(state.cutout.width,state.cutout.height,1100);c.width=s.width;c.height=s.height;const ctx=c.getContext("2d",{alpha:true});ctx.clearRect(0,0,c.width,c.height);ctx.imageSmoothingQuality="high";ctx.drawImage(state.cutout,0,0,c.width,c.height);return}
  const s=fit(state.source.width,state.source.height,1100);c.width=s.width;c.height=s.height;const ctx=c.getContext("2d",{alpha:true,willReadFrequently:true});ctx.clearRect(0,0,c.width,c.height);ctx.drawImage(state.source,0,0,c.width,c.height);const im=ctx.getImageData(0,0,c.width,c.height),d=im.data,sw=state.source.width,sh=state.source.height;
  for(let y=0;y<c.height;y++){const sy=Math.min(sh-1,Math.floor(y*sh/c.height));for(let x=0;x<c.width;x++){const sx=Math.min(sw-1,Math.floor(x*sw/c.width)),inside=state.mask[sy*sw+sx],i=(y*c.width+x)*4;if(!inside)d[i+3]=0;else{d[i+3]=255;if(state.manual){d[i]=d[i]*.72+198*.28;d[i+1]=d[i+1]*.72+255*.28;d[i+2]=d[i+2]*.72+53*.28}}}}ctx.clearRect(0,0,c.width,c.height);ctx.putImageData(im,0,0);
}
function renderFinal(){if(!state.binary)return;ui.finalCanvas.width=state.width;ui.finalCanvas.height=state.height;const ctx=ui.finalCanvas.getContext("2d",{alpha:true,willReadFrequently:true});ctx.clearRect(0,0,state.width,state.height);ctx.putImageData(toImageData(state.binary,state.width,state.height,state.inverted,state.finalMask,true),0,0);ui.invertBtn.classList.toggle("active",state.inverted)}
...
async function autoPrepare(){
  ...
  try{setProgress(8,"تحميل محرك Person Segmentation…");await tick();setProgress(18,"عزل الشخص الأساسي من الخلفية…");const segmented=await personMask();state.mask=segmented.mask;state.alpha=segmented.alpha;state.cutout=buildTransparentCutout(state.mask,state.alpha);setProgress(38,"تنظيف الحواف وإزالة الهالة البيضاء…");await tick();state.manual=false;ui.manualPanel.hidden=true;ui.maskCanvasWrap.classList.remove("editing");renderMask();await processMask(state.mask);showToast("تم عزل الشخص بخلفية شفافة وتنظيف الحواف وتجهيز نتيجة الحفر.")}
...
function exportCutoutPNG(){if(state.cutout)state.cutout.toBlob(b=>{if(b){const old=state.name;state.name=`${old}-transparent-cutout`;download(b,"png");state.name=old;showToast("تم تصدير الشخص فقط بخلفية شفافة.")}},"image/png")}
function exportPNG(){if(state.binary)ui.finalCanvas.toBlob(b=>{if(b){download(b,"png");showToast("تم تصدير PNG للحفر بخلفية شفافة.")}},"image/png")}
...
function exportSVG(){if(!state.binary)return;setProgress(30,"بناء مسارات SVG…");setTimeout(()=>{try{const svg=`<?xml version="1.0" encoding="UTF-8"?><svg xmlns="http://www.w3.org/2000/svg" width="${state.width}" height="${state.height}" viewBox="0 0 ${state.width} ${state.height}"><path d="${svgPath()}" fill="black"/></svg>`;download(new Blob([svg],{type:"image/svg+xml"}),"svg");setProgress(100,"تم إنشاء SVG vector-style");showToast("تم تصدير SVG بخلفية شفافة.")}catch(e){console.error(e);showToast("تعذر إنشاء SVG.",true)}endProgress()},30)}
...
function renderEye(){if(!state.eyeBinary)return;const c=ui.eyeCanvas;c.width=state.eyeWidth;c.height=state.eyeHeight;const im=new ImageData(c.width,c.height),d=im.data;for(let i=0,p=0;i<state.eyeBinary.length;i++,p+=4){d[p]=d[p+1]=d[p+2]=0;d[p+3]=state.eyeBinary[i]===0?255:0}const ctx=c.getContext("2d",{alpha:true});ctx.clearRect(0,0,c.width,c.height);ctx.putImageData(im,0,0)}
...
function eyeCrop(binary,w,h){
  const ink=new Uint8Array(binary.length);for(let i=0;i<ink.length;i++)ink[i]=binary[i]===0?1:0;const b=bounds(ink,w,h);if(!b||b.area<w*h*.0008)throw Error("No eye detail");const m=Math.max(5,Math.round(Math.max(b.w,b.h)*.09)),sx=Math.max(0,b.x-m),sy=Math.max(0,b.y-m),ex=Math.min(w,b.x+b.w+m),ey=Math.min(h,b.y+b.h+m),ow=ex-sx,oh=ey-sy,out=new Uint8Array(ow*oh);out.fill(255);for(let y=0;y<oh;y++)for(let x=0;x<ow;x++)out[y*ow+x]=binary[(y+sy)*w+x+sx];return{binary:out,w:ow,h:oh};
}
...
function exportEyePNG(){if(state.eyeBinary)ui.eyeCanvas.toBlob(b=>{if(b){const old=state.name;state.name=state.eyeName+"-eye-detail";download(b,"png");state.name=old;showToast("تم تصدير رسمة العين PNG شفافة.")}},"image/png")}
function exportEyeSVG(){if(!state.eyeBinary)return;const svg=`<?xml version="1.0" encoding="UTF-8"?><svg xmlns="http://www.w3.org/2000/svg" width="${state.eyeWidth}" height="${state.eyeHeight}" viewBox="0 0 ${state.eyeWidth} ${state.eyeHeight}"><path d="${eyeSvgPath()}" fill="black"/></svg>`,old=state.name;state.name=state.eyeName+"-eye-detail";download(new Blob([svg],{type:"image/svg+xml"}),"svg");state.name=old;showToast("تم تصدير فيكتور العين بخلفية شفافة.")}
