...
const ui = Object.fromEntries(["fileInput","dropZone","fileMeta","prepareBtn","modelBadge","progressWrap","progressBar","progressText","manualPanel","brushSize","resetMaskBtn","applyMaskBtn","emptyState","previewGrid","originalCanvas","maskCanvas","finalCanvas","maskCanvasWrap","resultStats","exportBar","invertBtn","pngBtn","bmpBtn","txtBtn","svgBtn","pdfBtn","sizeStat","faceStat","exportPxStat","toast","materialSelect","targetWidth","targetHeight","dpiSelect","lineSpacePreview","lockRatio","materialName","powerVal","speedVal","frequencyVal","lineSpaceVal","dpiVal","loopVal","settingsNote"].map(id => [id, $(id)]));
const state = { source:null, name:"portrait", mask:null, binary:null, width:0, height:0, segmenter:null, modelPromise:null, processing:false, inverted:false, manual:false, brushMode:"add", drawing:false, lastPoint:null, face:null, exportMeta:null, sourceProcessSize:null, toastTimer:0 };
...
async function ensureSegmenter(){
  ...
  state.modelPromise=(async()=>{try{
    const {FilesetResolver,ImageSegmenter}=await import(MP_MODULE);const vision=await FilesetResolver.forVisionTasks(MP_WASM);
    state.segmenter=await ImageSegmenter.createFromOptions(vision,{baseOptions:{modelAssetPath:PERSON_MODEL,delegate:"CPU"},runningMode:"IMAGE",outputCategoryMask:true,outputConfidenceMasks:false});
...
async function loadFile(file){
  ...
  try{const url=URL.createObjectURL(file),img=new Image();img.decoding="async";img.src=url;await img.decode();const natural=[img.naturalWidth,img.naturalHeight],size=fit(...natural),c=makeCanvas(size.width,size.height),ctx=c.getContext("2d",{alpha:false,willReadFrequently:true});
    ...
    Object.assign(state,{source:c,name:(file.name.replace(/\.[^.]+$/,"")||"portrait").replace(/[^\w\u0600-\u06ff-]+/g,"-"),mask:null,binary:null,inverted:false,manual:false,exportMeta:null,sourceProcessSize:null});
    ...
    ui.emptyState.hidden=true;ui.previewGrid.hidden=false;ui.resultStats.hidden=true;ui.exportBar.hidden=true;drawPreview(c,ui.originalCanvas);placeholders();switchView("all");showToast("تم تحميل الصورة. اضغط زر التحضير التلقائي.");updateEzcadPanel();ensureSegmenter().catch(()=>{});
...
async function personMask(){
  const seg=await ensureSegmenter(),result=seg.segment(state.source);if(!result?.categoryMask)throw Error("No mask");const w=state.source.width,h=state.source.height,cat=result.categoryMask,raw=cat.getAsUint8Array(),sw=cat.width,sh=cat.height,full=new Uint8Array(w*h);
  for(let y=0;y<h;y++){const src=Math.min(sh-1,Math.floor(y*sh/h))*sw;for(let x=0;x<w;x++)full[y*w+x]=raw[src+Math.min(sw-1,Math.floor(x*sw/w))]>0?1:0}cat.close?.();
  let clean=primary(full,w,h);if(!reliable(bounds(clean,w,h),w,h))throw Error("Weak mask");const p=currentPreset(),s=Math.max(w,h)/MAX_SIDE;clean=dilate(clean,w,h,Math.max(4,p.dilate*s));clean=erode(dilate(clean,w,h,Math.max(2,p.close*s)),w,h,Math.max(2,p.close*s));clean=fillHoles(clean,w,h,Math.max(80,Math.round(bounds(clean,w,h).area*.0025)));if(!reliable(bounds(clean,w,h),w,h))throw Error("Rejected mask");return clean;
}

function cropPerson(mask){
  const w=state.source.width,h=state.source.height,b=bounds(mask,w,h);if(!b)throw Error("Empty mask");const m=Math.round(Math.max(b.w,b.h)*.10),ow=b.w+2*m,oh=b.h+2*m,sx=b.x-m,sy=b.y-m,c=makeCanvas(ow,oh),ctx=c.getContext("2d",{alpha:false,willReadFrequently:true}),cm=new Uint8Array(ow*oh);
...
function toImageData(binary,w,h,invert=false){const im=new ImageData(w,h),d=im.data;for(let i=0,p=0;i<binary.length;i++,p+=4){const v=invert?255-binary[i]:binary[i];d[p]=d[p+1]=d[p+2]=v;d[p+3]=255}return im}

function renderMask(){
  if(!state.source||!state.mask)return;const s=fit(state.source.width,state.source.height,1100),c=ui.maskCanvas;c.width=s.width;c.height=s.height;const ctx=c.getContext("2d",{alpha:false,willReadFrequently:true});ctx.fillStyle="white";ctx.fillRect(0,0,c.width,c.height);ctx.drawImage(state.source,0,0,c.width,c.height);const im=ctx.getImageData(0,0,c.width,c.height),d=im.data,sw=state.source.width,sh=state.source.height;
  for(let y=0;y<c.height;y++){const sy=Math.min(sh-1,Math.floor(y*sh/c.height));for(let x=0;x<c.width;x++){const sx=Math.min(sw-1,Math.floor(x*sw/c.width)),inside=state.mask[sy*sw+sx],i=(y*c.width+x)*4;if(!inside)d[i]=d[i+1]=d[i+2]=255;else if(state.manual){d[i]=d[i]*.72+198*.28;d[i+1]=d[i+1]*.72+255*.28;d[i+2]=d[i+2]*.72+53*.28}}}ctx.putImageData(im,0,0);
}
function renderFinal(){if(!state.binary)return;ui.finalCanvas.width=state.width;ui.finalCanvas.height=state.height;ui.finalCanvas.getContext("2d",{alpha:false,willReadFrequently:true}).putImageData(toImageData(state.binary,state.width,state.height,state.inverted),0,0);ui.invertBtn.classList.toggle("active",state.inverted)}
function pureBinary(){if(!state.binary)return false;const d=ui.finalCanvas.getContext("2d",{willReadFrequently:true}).getImageData(0,0,state.width,state.height).data;for(let i=0;i<d.length;i+=4)if(![0,255].includes(d[i])||d[i+1]!==d[i]||d[i+2]!==d[i]||d[i+3]!==255)return false;return true}

async function processMask(mask){
  setProgress(48,"قص الشخص مع هامش أمان 10%…");await tick();const crop=cropPerson(mask);setProgress(57,"تحديد الوجه وحماية الملامح…");await tick();const face=await faceRect(crop),data=crop.source.getContext("2d",{willReadFrequently:true}).getImageData(0,0,crop.w,crop.h),gray=grayscale(data,crop.mask),p=currentPreset();
  ...
  setProgress(92,"تحجيم الملف على مقاس EZCAD والـ DPI المختار…");await tick();const target=readTargetSize(crop.w,crop.h),sized=resizeBinaryNearest(binary,crop.w,crop.h,target.w,target.h);Object.assign(state,{binary:sized,width:target.w,height:target.h,face,inverted:false,sourceProcessSize:{w:crop.w,h:crop.h},exportMeta:{...target,material:ui.materialSelect?.value||"steel_photo"}});
...
async function autoPrepare(){
  ...
  try{setProgress(8,"تحميل محرك Person Segmentation…");await tick();setProgress(18,"عزل الشخص الأساسي من الخلفية…");state.mask=await personMask();setProgress(38,"توسيع الماسك وغلق الفراغات وحماية الأطراف…");await tick();state.manual=false;ui.manualPanel.hidden=true;ui.maskCanvasWrap.classList.remove("editing");renderMask();await processMask(state.mask);showToast("تم تجهيز البورتريه — الخلفية أبيض والناتج لونان فقط.")}
...
function manualMode(){state.mask=new Uint8Array(state.source.width*state.source.height);state.mask.fill(1);state.manual=true;state.binary=null;ui.manualPanel.hidden=false;ui.maskCanvasWrap.classList.add("editing");ui.resultStats.hidden=true;ui.exportBar.hidden=true;ui.invertBtn.disabled=true;renderMask();switchView("mask")}
async function applyManual(){if(state.processing||!state.mask)return;const b=bounds(state.mask,state.source.width,state.source.height);if(!b||b.area<state.mask.length*.01)return showToast("الماسك صغير جدًا. لوّن الشخص كاملًا أولًا.",true);state.processing=true;ui.applyMaskBtn.disabled=true;ui.prepareBtn.disabled=true;try{setProgress(35,"اعتماد الماسك اليدوي بدون قص إضافي…");await tick();await processMask(state.mask);showToast("تم تطبيق الماسك وتجهيز الملف للحفر.")}catch(e){console.error(e);showToast("تعذرت المعالجة. راجع الماسك وحاول مرة أخرى.",true)}finally{state.processing=false;ui.applyMaskBtn.disabled=false;ui.prepareBtn.disabled=false;endProgress()}}
...
function exportPNG(){if(state.binary)ui.finalCanvas.toBlob(b=>{if(b){download(b,"png");showToast("تم تصدير PNG جاهز للحفر في EZCAD.")}},"image/png")}
...
function svgPath(){const done=[];let active=new Map();for(let y=0;y<state.height;y++){const runs=[];let start=-1;for(let x=0;x<=state.width;x++){const black=x<state.width&&(state.inverted?state.binary[y*state.width+x]===255:state.binary[y*state.width+x]===0);if(black&&start<0)start=x;if(!black&&start>=0){runs.push([start,x-start]);start=-1}}const next=new Map();for(const[x,w]of runs){const key=`${x}:${w}`,r=active.get(key)||{x,y,w,h:0};r.h++;next.set(key,r)}for(const[k,r]of active)if(!next.has(k))done.push(r);active=next}done.push(...active.values());return done.map(r=>`M${r.x} ${r.y}h${r.w}v${r.h}h-${r.w}Z`).join("")}
function exportSVG(){if(!state.binary)return;setProgress(30,"بناء مسارات SVG…");setTimeout(()=>{try{const svg=`<?xml version="1.0" encoding="UTF-8"?><svg xmlns="http://www.w3.org/2000/svg" width="${state.width}" height="${state.height}" viewBox="0 0 ${state.width} ${state.height}"><rect width="100%" height="100%" fill="white"/><path d="${svgPath()}" fill="black"/></svg>`;download(new Blob([svg],{type:"image/svg+xml"}),"svg");setProgress(100,"تم إنشاء SVG vector-style");showToast("تم تصدير SVG بمسارات فيكتور سوداء.")}catch(e){console.error(e);showToast("تعذر إنشاء SVG.",true)}endProgress()},30)}
...
updateEzcadPanel();
modelStatus("idle",location.protocol==="file:"?"شغّل من localhost لتحميل الموديل":"الموديل يُحمّل عند أول صورة");
if(location.protocol!=="file:")ensureSegmenter().catch(()=>{});
window.__fiberAutoMax={get state(){return state},verifyPureBinary:pureBinary,presets:Object.keys(PRESETS),materials:Object.keys(MATERIALS),version:"4.1.0-ezcad"};
