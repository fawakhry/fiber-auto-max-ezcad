// MATBAAGY FIBER LASER 5.7.0 — production accuracy fix
const APP_VERSION = "5.7.0";
const REMOTE_HOST = /^https?:$/.test(location.protocol) && !["localhost","127.0.0.1","[::1]"].includes(location.hostname);
const DEMO_MODE = new URLSearchParams(location.search).get("demo") === "1" || window.MATBAAGY_CONFIG?.edition === "demo" || (REMOTE_HOST && window.MATBAAGY_CONFIG?.allowHostedFull !== true);
...
async function processMask(mask){
  ...
  setProgress(92,"تحجيم الملف على مقاس EZCAD والـ DPI المختار…");await tick();const target=readTargetSize(crop.w,crop.h);if(!target.valid){const error=Error(target.error);error.code="INVALID_EXPORT_SIZE";throw error}const sized=resizeBinaryNearest(binary,crop.w,crop.h,target.w,target.h),finalMask=resizeMaskNearest(crop.mask,crop.w,crop.h,target.w,target.h);Object.assign(state,{binary:sized,finalMask,width:target.w,height:target.h,face,inverted:false,sourceProcessSize:{w:crop.w,h:crop.h},exportMeta:{...target,material:ui.materialSelect?.value||"steel_photo"}});
...
function exportCutoutPNG(){if(demoGuard())return;if(state.cutout)state.cutout.toBlob(b=>{if(b){download(b,"png",`${state.name}-white-cutout`);showToast("تم تصدير الشخص فقط بخلفية بيضاء وبدون فريم زائد.")}},"image/png")}
async function exportPNG(){if(demoGuard()||!state.binary)return;try{download(await canvasToPngBlobWithDpi(ui.finalCanvas,state.exportMeta?.dpi||getDpi()),"png");showToast(`تم تصدير PNG بدقة ${state.exportMeta?.dpi||getDpi()} DPI.`)}catch(e){console.error(e);showToast("تعذر إنشاء PNG.",true)}}
...
function exportBMP(){if(demoGuard()||!state.binary)return;const dpi=state.exportMeta?.dpi||getDpi();download(encodeBMP(ui.finalCanvas,dpi),"bmp");showToast(`تم تصدير BMP بدقة ${dpi} DPI للإدخال في EZCAD.`)}
...
async function exportEyePNG(){if(demoGuard()||!state.eyeBinary)return;try{const dpi=state.eyeExportMeta?.dpi||clamp(+ui.eyeDpiSelect.value||300,300,600),blob=await canvasToPngBlobWithDpi(ui.eyeCanvas,dpi);download(blob,"png",`${state.eyeName}-wallet-eye`);showToast(`تم تصدير رسمة العين PNG بالمقاس الحقيقي و${dpi} DPI.`)}catch(e){console.error(e);showToast("تعذر إنشاء PNG للعين.",true)}}
function exportEyeSVG(){if(demoGuard()||!state.eyeBinary)return;try{const meta=state.eyeExportMeta||{wmm:pxToMm(state.eyeWidth,+ui.eyeDpiSelect.value||300),hmm:pxToMm(state.eyeHeight,+ui.eyeDpiSelect.value||300)},svg=`<?xml version="1.0" encoding="UTF-8"?><svg xmlns="http://www.w3.org/2000/svg" width="${meta.wmm.toFixed(3)}mm" height="${meta.hmm.toFixed(3)}mm" viewBox="0 0 ${state.eyeWidth} ${state.eyeHeight}"><rect width="100%" height="100%" fill="white"/><path d="${eyeSvgPath()}" fill="black"/></svg>`;download(new Blob([svg],{type:"image/svg+xml"}),"svg",`${state.eyeName}-wallet-eye`);showToast("تم تصدير SVG للعين بالمقاس الحقيقي.")}catch(e){console.error(e);showToast(e?.message==="SVG_TOO_COMPLEX"?"تفاصيل العين كثيفة جدًا لـ SVG. استخدم PNG للحفر.":"تعذر إنشاء SVG للعين.",true)}}
...
window.__fiberAutoMax={get state(){return state},selectMode,verifyPureBinary:pureBinary,presets:Object.keys(PRESETS),materials:Object.keys(MATERIALS),version:APP_VERSION,demo:DEMO_MODE,test:{mmToPx,pxToMm,validatePixelSize,encodeBMP,canvasToPngBlobWithDpi,compareVersions}};
